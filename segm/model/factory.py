...
def create_vit(model_cfg):
    ...
    if backbone == 'vit_tiny_patch16_384':
        ...
        pretrained_vit_pth = os.path.join(os.getcwd(), 'resources/vit_tiny_patch16_384.npz')
        model.load_pretrained(pretrained_vit_pth)

    elif backbone == 'vit_small_patch16_384':
        print('backbone:', backbone)
        pretrained_vit_pth = os.path.join(os.getcwd(), 'resources/vit_small_patch16_384.npz')
        model.load_pretrained(pretrained_vit_pth)

    elif backbone == 'vit_base_patch16_384':
        print('backbone:', backbone)
        pretrained_vit_pth = os.path.join(os.getcwd(), 'resources/vit_base_patch16_384.npz')
        model.load_pretrained(pretrained_vit_pth)

    elif backbone == 'vit_large_patch16_384':
        print('backbone:', backbone)
        pretrained_vit_pth = os.path.join(os.getcwd(), 'resources/vit_large_patch16_384.npz')
        model.load_pretrained(pretrained_vit_pth)
    elif "deit" in backbone:
        load_pretrained(model, default_cfg, filter_fn=checkpoint_filter_fn)
    else:
        pretrained_vit_pth = os.path.join(os.getcwd(), 'resources/vit_tiny_patch16_224.npz')
        model.load_pretrained(pretrained_vit_pth)
