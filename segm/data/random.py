...
class RandomDataset(Dataset):
    def __init__(
        ...
        if self.add_mask:
            assert os.path.exists(
                os.path.join(self.dataset_dir, 'mask_prior.pickle'))
    ...
    def get_img(self, key):
        ...
        if self.add_mask:
            ...
            mask_p_c = np.zeros((self.crop_size ** 2, self.n_cls),
                                dtype=np.float32)

            for l_range in range(self.mask_num):
                start_l1, end_l1 = l_range * (100 // self.mask_num), (
                            l_range + 1) * (100 // self.mask_num)
                if end_l1 == 100:
