...
class COCODataset(Dataset):
    def __init__(
        ...
        self.gray_image_dir = gray_image_dir
    ...
    def get_img(self, key):
        if self.split == 'train':
            ...
            # gray_image_path = os.path.join(self.gray_image_dir, key[1])
            # gray_image_path = os.path.join(self.gray_image_dir, str(int(key[1].split(".")[0]))+".png")
        else:
            img_pth = os.path.join(self.image_dir, key)
            gray_image_path = os.path.join(self.gray_image_dir, key)
        img = Image.open(img_pth).convert("RGB")
        # gray_image = Image.open(gray_image_path).convert("RGB")
        w, h = img.size
        if self.split == 'train':
            mini_size = min(self.crop_size, self.crop_size)
        ...
        # gray_image_resized = img_transform(gray_image)
        # gray_image_resized = np.array(gray_image_resized)


        l_resized = self.rgb_to_lab(img_resized)[:, :, :1]
        # l_resized = self.rgb_to_lab(gray_image_resized)[:, :, :1]
