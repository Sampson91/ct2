...
@click.option("--im-size", default=16 * 10, type=int,
...
# @click.option('--save_dir', type=str, default='data/CT2_output',help="test image directory")
@click.option('--save_dir', type=str, default='', help="test image directory")
@click.option("--origin_image_path", type=str, default='', help="origin image path")
@click.option("--save_image_concat_dir", type=str, default='', help="origin image path")
def main(
        log_dir,
        dataset,
        dataset_dir,
        ...
        save_dir,
        origin_image_path,
        save_image_concat_dir
):
    print('folders are checked')
    # convert point clouds to indexs and images

    # process_image_split(origin_image_path, dataset_dir, im_size)
    # convert rgb to gray for testing
    ...
    if resume and checkpoint_exist:
        ...
        with open(checkpoint_path, 'rb') as f:
            ...
            # model.load_state_dict(net_state_dict["model"])
    if ptu.distributed:
    ...
    # sys.exit(1)
    # process_test_image_concat(origin_image_path, save_dir, save_image_concat_dir,im_size)
